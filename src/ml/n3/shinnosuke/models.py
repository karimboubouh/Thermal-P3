...
class Sequential(BaseModel):
    ...
    def fit(self, X, Y, batch_size=64, epochs=20, shuffle=True, validation_data=None, validation_ratio=0.1,
        ...
        for epoch in range(epochs):
            ...
            trained_nums = 0
            if verbose:
                log('\033[0;31m Epoch[%d/%d]\33[0m' % (epoch + 1, epochs))
        ...
        if validation_data is not None:
            ...
            if verbose:
                log(f"\33[34mValidation >> MSE : {v_mse:.4f}, RMSE : {v_rmse:.4f}, MAE : {v_mae:.4f}\33[0m")
...
class Model(BaseModel):
    ...
    def fit(self, X, Y, batch_size=64, epochs=20, shuffle=True, validation_data=None, validation_ratio=0.1,
        ...
        for epoch in range(epochs):
            ...
            batch_count = 0
            trained_nums = 0
            log('\033[0;31m Epoch[%d/%d]' % (epoch + 1, epochs))
            start_time = time.time()
            # T = tqdm(range(len(mini_batches)), position=0)
            # for t in T:
            #     xs, ys = mini_batches[t]
            for xs, ys in mini_batches:
                ...
                if validation_data is not None:
                    ...
                    log('\r{:d}/{:d} [{}] -{} -{}/batch -batch_loss: {:.4f} -batch_acc: {:.4f} '.format(trained_nums,
                                                                                                        training_size,
                                                                                                        process_bar,
                                                                                                        self.format_time(
                                                                                                            gap),
