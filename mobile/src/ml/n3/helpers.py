...
def model_fit(peer, tqdm_bar=False):
    ...
    history = peer.model.fit(X, Y, shuffle=False, epochs=peer.params.epochs, validation_data=val,
                             batch_size=peer.params.batch_size, log=peer.log)
    h = list(history.values())
    peer.log('result', f"Node {peer.id} Train MSE: {h[0][-1]:4f}, RMSE: {h[1][-1]:4f} | MAE {h[2][-1]:4f}")
...
def meta_train(i, model_file, train, batch_size, epochs=1):
    ...
    peer.log('event', f"Home {i} performs personalized learning using local data for {epochs} epochs...")
    model = load_model(model_file)
    model.fit(train, epochs=epochs, batch_size=batch_size, log=peer.log)
    history = model.history.history
    h = list(history.values())
    peer.log('success', f"Node {i} META Train MSE: {h[0][-1]:4f}, RMSE: {h[1][-1]:4f} | MAE {h[2][-1]:4f}")
...
def train_for_x_batches(peer, batches=1, evaluate=False):
    ...
    for _ in range(batches):
        ...
        if evaluate:
            h = peer.model.fit(bX, bY, validation_data=val, shuffle=False, epochs=1, batch_size=len(bY), verbose=False,
                               log=peer.log)
            ...
            h = peer.model.fit(bX, bY, validation_ratio=0, shuffle=False, epochs=1, batch_size=len(bY), verbose=False,
                               log=peer.log)
            h1.loss.append(h['loss'])
            h1.rmse.append(h['rmse'])
            h1.mae.append(h['mae'])
...
def evaluate_home(home_id, model, generator, batch_size=16, one_batch=False, dtype="Test "):
    ...
    # todo :: update
    peer = Map({'log': "ERROR"})
    peer.log('result', f"Home {home_id} || {dtype} {one} MSE: {h[0]:4f} | RMSE: {h[1]:4f}, MAE: {h[2]:4f}")
    return history
...
def get_params(model, named=False, numpy=False):
    if named:
        # todo :: update
        peer = Map({'log': "ERROR"})
        peer.log("error", "Getting params using named params is not supported")
        exit()
...
def set_params(model, params, named=False, numpy=None):
    ...
    if named:
        # todo :: update
        peer = Map({'log': "ERROR"})
        peer.log("error", "Setting params using named params is not supported")
        exit()
...
def model_predict(model, generator):
    ...
    # todo :: update
    peer = Map({'log': "ERROR"})
    peer.log('info', f"Prediction for {test_size} entries...")
